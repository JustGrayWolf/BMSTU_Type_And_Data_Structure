...
int main()
{
    ...
    if (type == 1)
    {
        ...
        double total_t_out1 = TIME * av_t_out1 * CHANCE_MAX/(CHANCE_MAX - 80);
        ...
        printf("Среднее время прихода: %f\n"
               ...
               av_t_in1, av_t_out1, total_t_in1 > total_t_out1 ? total_t_in1 : total_t_out1, total_t_out1);
		...
		printf("Реальное время работы программы: %d ms", realtime);
        queue_free(q1);
        return 0;
    }
    else if (type == 2)
